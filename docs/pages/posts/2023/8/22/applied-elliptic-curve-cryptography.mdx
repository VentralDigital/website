...
* For such a group to be complete, an operation is required under which several properties hold true. In the case of ECC, this operation is usually referred to as "point addition", but it doesn't actually have anything to do with any sort of mathematical addition, rather the operation was "made up" to fit the rules.
    * A point $`P`$ on the curve "added" to itself ($`P_1 + P_1 = P_2`$) is defined as an operation that draws a tangent on the curve through $`P`$. Thanks to the elliptic curve's nature, this tangent line will cross the curve at exactly one other point, which is mirrored over the x-axis and then declared to be the "result of the addition", point $`P_2`$.
    ...
    * Scalars are normal integers and not part of the group, which means that between scalars any usual mathematical operation is available. But it should be noted that, where calculations between scalars are involved, these are typically done with modular arithmetic over $`("mod "n)`$ where $`n`$ is the order of the cyclic group (ie. the total number of elements in the group, not the same as the prime $`p`$ defining the finite field).
...
In RSA, numbers become cyclical thanks to modular arithmetic and the use of prime numbers. Similarly, when the order (ie. the number of existing points) of the elliptic curve is prime, almost any non-identity point on the curve can serve as a generator point $`G`$ (also referred to as "primitive element", or "base point"). A generator, when "added" to itself over and over again, will cycle through all elements of the group ($`G, 2G, 3G, ..., (n-1)G, cc"O", G, 2G, ...`$). However, not all curves have a prime order. For those that have a composite order, it's common to choose a large prime-order subgroup and a generator that is part of this sub-group is then determined. These generators won't iterate through all points on the curve, only through points in their subgroup. If the subgroup's order is a large prime, it will have a number of elements comparable to the entire curve.

In RSA, the "trapdoor function" relied on the mathematical property that once two large random prime factors are multiplied, it is difficult to factorize the product in order to recover them. The discrete logarithmic problem in ECC (ECDLP) instead relies on the fact that it's easier to determine a point $`P`$ by multiplying a scalar with a generator ($`P = sG`$) than it is by determining the scalar that was used from knowing both $`P`$ and $`G`$. This operation can be efficiently executed forward thanks to methods such as the Double-And-Add algorithm, while the reverse requires a much more brute force approach.

One of the greatest advantages of ECC compared to RSA is that the secret scalar in ECDLP may be much smaller than the prime factors chosen in RSA without a sacrifice in security, making it a more space-saving and efficient scheme. It can also be argued that ECC has fewer footguns, which doesn't mean that choosing the right curve and a good generator point isn't important for the scheme's security.
...
$`(s_1 = k - h(m_1, R)*a)`$ minus $`(s_2 = k - h(m_2, R)*a ) ->`$

$`s_1 - s_2 = (k - h(m_1, R)*a) - (k - h(m_2, R)*a)`$

$`s_1 - s_2 = k - h(m_1, R)*a - k + h(m_2, R)*a`$

$`s_1 - s_2 = cancel((k - k)) + (h(m_2, R)*a - h(m_1, R)*a)`$

$`s_1 - s_2 = a*(h(m_2, R) - h(m_1, R))`$

$`(s_1 - s_2) / (h(m_2, R) - h(m_1, R)) = a`$
...
# Key recovery: a = (s1 - s2) / (h(m2, R) - h(m1, R))
