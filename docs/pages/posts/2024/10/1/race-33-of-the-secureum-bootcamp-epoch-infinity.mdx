...
**A/C**: Assuming the `unlockedAt` timestamp has expired, a user can call `withdraw()` directly without previously calling `prepareWithdraw()`, allowing them to obtain arbitrary amounts. This is works because the withdrawals amount is re-set to 0 instead of subtracting the withdrawal amount from it, which would cause a revert from integer underflow protection if a user attempts to withdraw more than deposited.
