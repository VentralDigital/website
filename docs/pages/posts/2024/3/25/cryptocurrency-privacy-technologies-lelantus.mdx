...
<Head>
  <meta property="og:image" content={'https://ventral.digital' + thumbnail.src} />
...
lelantus bug: "something like generate coin A, coin B, then set up the state as if both were minted (but mine only one mint), then spend the second coin using the forged state"

"And yes, it's the case that Spark doesn't need or use internal counters to maintain state
Coins are instantiated using random nonces that the recipient recovers as part of its detection and correctness checks"
