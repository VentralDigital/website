# The Secureum Bootcamp

$$
`tt"Secureum" = tt"Security" + tt"Ethereum"`
$$

import { ArticleCard, ArticleCards } from '@components/articlecards'
import { NewsletterIcon, GlobeIcon } from '@components/icons'
import { Callout } from 'nextra/components'
import Head from 'next/head'
import banner from './secureum-banner.jpg'

<Head>
  <meta property="og:image" content={'https://ventral.digital' + banner.src} />
  <meta name="twitter:image" content={'https://ventral.digital' + banner.src} />
</Head>

## When is the next bootcamp gonna start?

**~~TL;DR: It's running! It's online! You can start right away! Check [#participate](https://discord.com/channels/814328279468474419/928441092116975696) for details!~~**

Confused? Well, let me explain...

The Secureum Bootcamp started out in October 2021 with "Epoch 0" and was divided into LEARN and CARE phase. The LEARN phase ran for 8 weeks and each week had its own "slot". Learning materials for each slot was released week by week and later tested in quizzes.

In December, the 128 best scoring participants (from 1024 total participants) were invited to the CARE phase. Each participant was randomly assigned to one of four projects that partnered up with Secureum for an "audit-like" contest. During this contest participants would review the project's provided code (as one would during an audit) and provide a report of any findings which were aggregated into a single big report in the end.

In 2022, with all of the learning material from Epoch 0 already available, Epoch Infinity was launched. The bootcamp is no longer guided, meaning no more weekly emails with learning material. Instead, participants are expected to learn the materials from Epoch 0 at their own pace. Then, once they feel ready, they can sign up for the next RACE. These are monthly quizzes that will test your knowledge compared to other participants. Sometimes the top scorers of these quizes will be invited to special events like CARE or CARE-X.

<Callout type="info">
  In August 2025 Epoch Infinity has been paused. For now, there'll be no further monthly RACEs (See [official announcement](https://x.com/0xRajeev/status/1954441182735397229)). You may however still review the contents of previous RACEs listed below.
</Callout>

* You can relive Bootcamp Epoch 0 in [Discord](https://discord.com/channels/814328279468474419/866157287424917564/866359765311160330) to get the original experience or alternatively find a summary of relevant material in the [Secureum Mindmap](https://github.com/x676f64/secureum-mind_map)
* You can practice for RACEs using the quiz write-ups listed below.
* ~~Once you feel ready to [#participate](https://discord.com/channels/814328279468474419/928441092116975696) in RACEs, don't forget to sign up. Pay attention to the *Important Dates* to see when registrations open and when the RACE will happen. Note that registration is only required once, if you already have a RUN code you only need to monitor the [#announcements](https://discord.com/channels/814328279468474419/927065287172427798) channel for the link to the quiz page. There will NOT be reminder e-mail about the monthly RACEs, you have to check the channels!~~

## Write-Ups

<ArticleCards>
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #42"
    subtitle="July 14, 2025 by patrickd"
    href="/posts/2025/7/14/race-42-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #41"
    subtitle="June 19, 2025 by patrickd"
    href="/posts/2025/6/19/race-41-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #40"
    subtitle="May 12, 2025 by patrickd"
    href="/posts/2025/5/12/race-40-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<GlobeIcon />}
    title="Epoch∞ RACE #39"
    subtitle="By trocher (Mirrored)"
    href="/posts/2025/4/7/race-39-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #38"
    subtitle="March 3, 2025 by windhustler"
    href="/posts/2025/3/3/race-38-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #37"
    subtitle="February 14, 2025 by patrickd"
    href="/posts/2025/2/14/race-37-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #36"
    subtitle="January 10, 2025 by Zigtur"
    href="/posts/2025/1/10/race-36-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #35"
    subtitle="December 10, 2024 by patrickd"
    href="/posts/2024/12/10/race-35-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #34"
    subtitle="November 10, 2024 by patrickd"
    href="/posts/2024/11/10/race-34-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #33"
    subtitle="October 1, 2024 by patrickd"
    href="/posts/2024/10/1/race-33-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #32"
    subtitle="September 8, 2024 by patrickd"
    href="/posts/2024/9/8/race-32-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #31"
    subtitle="July 30, 2024 by patrickd"
    href="/posts/2024/7/30/race-31-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #30"
    subtitle="June 24, 2024 by patrickd"
    href="/posts/2024/6/24/race-30-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<GlobeIcon />}
    title="Epoch∞ RACE #29"
    subtitle="By kamensec (Mirrored)"
    href="/posts/2024/5/15/race-29-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #28"
    subtitle="April 2, 2024 by patrickd"
    href="/posts/2024/4/2/race-28-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #27"
    subtitle="March 4, 2024 by patrickd"
    href="/posts/2024/3/4/race-27-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #26"
    subtitle="January 30, 2024 by patrickd"
    href="/posts/2024/1/30/race-26-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #25"
    subtitle="January 1, 2024 by patrickd"
    href="/posts/2024/1/1/race-25-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<GlobeIcon />}
    title="Epoch∞ RACE #24"
    subtitle="By Hexens (Mirrored)"
    href="/posts/2023/12/17/race-24-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<GlobeIcon />}
    title="Epoch∞ RACE #23"
    subtitle="By Veridise (Mirrored)"
    href="/posts/2023/11/8/race-23-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #22"
    subtitle="October 3, 2023 by patrickd"
    href="/posts/2023/10/3/race-22-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #21"
    subtitle="September 9, 2023 by patrickd"
    href="/posts/2023/9/5/race-21-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #20"
    subtitle="July 27, 2023 by patrickd"
    href="/posts/2023/7/30/race-20-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Secureum A-MAZE-X CTF 2023 At DeFi Security Summit"
    subtitle="July 17, 2023 by patrickd"
    href="/posts/2023/7/16/secureum-a-maze-x-ctf-2023-at-defi-security-summit/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #19"
    subtitle="July 1, 2023 by patrickd"
    href="/posts/2023/7/3/race-19-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #18"
    subtitle="May 25, 2023 by patrickd"
    href="/posts/2023/5/29/race-18-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #17"
    subtitle="May 1, 2023 by patrickd"
    href="/posts/2023/5/1/race-17-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #16"
    subtitle="March 25, 2023 by patrickd"
    href="/posts/2023/4/1/race-16-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #15"
    subtitle="February 22, 2023 by patrickd"
    href="/posts/2023/2/27/race-15-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #14"
    subtitle="January 30, 2023 by patrickd"
    href="/posts/2023/1/30/race-14-of-the-secureum-bootcamp-epoch-infinity/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #13"
    subtitle="January 3, 2023 by patrickd"
    href="/posts/2023/1/3/race-13-of-the-secureum-bootcamp-epoch/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #12"
    subtitle="December 6, 2022 by patrickd"
    href="/posts/2022/12/6/race-12-of-the-secureum-bootcamp-epoch/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #11"
    subtitle="November 1, 2022 by patrickd"
    href="/posts/2022/10/31/race-11-of-the-secureum-bootcamp-epoch/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #10"
    subtitle="October 3, 2022 by patrickd"
    href="/posts/2022/10/3/secureum-bootcamp-epoch-october-race-10/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #9"
    subtitle="August 31, 2022 by patrickd"
    href="/posts/2022/8/29/secureum-bootcamp-epoch-august-race-9/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Secureum A-MAZE-X Stanford CTF"
    subtitle="August 27, 2022 by patrickd"
    href="/posts/2022/8/27/secureum-a-maze-x-stanford-ctf/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #8"
    subtitle="August 2, 2022 by patrickd"
    href="/posts/2022/7/25/secureum-bootcamp-epoch-july-race-8/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #7"
    subtitle="July 5, 2022 by patrickd"
    href="/posts/2022/07/secureum-bootcamp-epoch-june-race-7/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #6"
    subtitle="May 17, 2022 by patrickd"
    href="/posts/2022/5/16/secureum-bootcamp-epoch-may-race-6/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #5"
    subtitle="March 8, 2022 by patrickd"
    href="/posts/2022/3/8/secureum-bootcamp-epoch-march-race-5/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="CARE: Sushi's BentoBox Strategies"
    subtitle="February 10, 2022 by patrickd"
    href="/posts/2022/2/10/secureum-bootcamp-care-audit-sushi-bentobox-strategies/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Epoch∞ RACE #4"
    subtitle="February 9, 2022 by patrickd"
    href="/posts/2022/2/9/secureum-bootcamp-february-race-4/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Audit Findings 201 Quiz (RACE #3)"
    subtitle="November 28, 2021 by patrickd"
    href="/posts/2021/11/28/secureum-bootcamp-audit-findings-201-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Audit Findings 101 Quiz (RACE #2)"
    subtitle="November 28, 2021 by patrickd"
    href="/posts/2021/11/28/secureum-bootcamp-audit-findings-101-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Audit Techniques & Tools 101 Quiz"
    subtitle="November 21, 2021 by patrickd"
    href="/posts/2021/11/21/secureum-bootcamp-audit-techniques-tools-101-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Security Pitfalls & Best Practices 201 Quiz (RACE #1)"
    subtitle="November 14, 2021 by patrickd"
    href="/posts/2021/11/14/secureum-bootcamp-security-pitfalls-amp-best-practices-201-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Security Pitfalls & Best Practices 101 Quiz (RACE #0)"
    subtitle="November 8, 2021 by patrickd"
    href="/posts/2021/11/7/secureum-bootcamp-security-pitfalls-amp-best-practices-101-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Solidity 201 Quiz"
    subtitle="October 31, 2021 by patrickd"
    href="/posts/2021/10/30/secureum-bootcamp-solidity-201-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Solidity 101 Quiz"
    subtitle="October 24, 2021 by patrickd"
    href="/posts/2021/10/24/secureum-bootcamp-solidity-101-quiz/"
  />
  <ArticleCard
    icon={<NewsletterIcon />}
    title="Ethereum 101 Quiz"
    subtitle="October 18, 2021 by patrickd"
    href="/posts/2021/10/17/secureum-bootcamp-ethereum-101-quiz/"
  />
</ArticleCards>